...
if __name__ == "__main__":
    ...
    reward_weights = {"delta_sigma": 70.0, "delta_norm": 100.0, "soc": 60.0}
    policy_kwargs = dict(
    activation_fn=th.nn.Tanh,   # opcional, padrão já é ReLU
    net_arch=[dict(
        pi=[64, 64],          # camadas da policy (π)
        vf=[64, 64]           # camadas da value function (V)
    )],
)
    ...
    ######## ----------------------- Treinamento ------------------- ################
    model = A2C("MlpPolicy", env_train, verbose=1, n_steps=steps_por_episodio)
    ...
    model.save(os.path.join(dir_model, "A2C_gridflex"))

    ######## ----------------------- Teste ------------------- ################

    print(f"Avaliando o agente RL treinado em {test_start} até {full_end}...")

    model = A2C.load(os.path.join(dir_model, "A2C_gridflex"), env=env_test)
    ...
    results["demand"].to_csv(
        os.path.join(output_csv + "demand/", "rl_forecasting_results_test_A2C.csv"),
        index=False,
    )
    results["bess"].to_csv(
        os.path.join(output_csv + "bess/", "rl_forecasting_bess_power_test_A2C.csv"),
        index=False,
    )
    rewards.to_csv(
        os.path.join(dir_model, "rl_forecasting_rewards_test_A2C.csv"), index=False
    )
